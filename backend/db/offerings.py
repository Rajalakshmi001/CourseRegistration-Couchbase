...
def offeringGet(quarterId, courseId, sectionId):
    ob_data = offering_bucket.lookup_in(quarterId, subdoc.get(courseId+"."+sectionId))
...
def offeringPost(quarterId, courseId, sectionId):
    offering_bucket.mutate_in(quarterId, subdoc.insert(courseId+"."+sectionId, request.get_json()))
